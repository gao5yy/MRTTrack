...
from lib.models.select_track.vit_select import vit_base_patch16_224_select
from lib.utils.box_ops import box_xyxy_to_cxcywh


class SelectTrack(nn.Module):
    """ This is the base class for SelectTrack developed on OSTrack (Ye et al. ECCV 2022) """
    ...
    def forward(self, template: torch.Tensor,
                search: torch.Tensor,
                template_mask_zv=None,template_mask_zi=None,
                ):
        x, aux_dict, decisions, loss_sel = self.backbone(z=template, x=search,
                                    template_mask_zv=template_mask_zv,template_mask_zi=template_mask_zi
                                    )
...
def build_select_track(cfg, training=True):
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224_select':
        backbone = vit_base_patch16_224_select(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE,
                                            select_loc=cfg.MODEL.BACKBONE.SELECT_LOC,
                                            select_drop_path=cfg.TRAIN.SELECT_DROP_PATH
                                            )
    else:
        raise NotImplementedError
