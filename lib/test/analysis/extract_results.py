    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            if os.path.isfile(results_path):
                ...
                # pred_bb_sampled = pred_bb[::10, :]                                               # 10倍采样
                # last_sample = pred_bb[-1, :].reshape(1, -1)
                # pred_bb = torch.cat((pred_bb_sampled, last_sample), dim=0)
            else:
                if skip_missing_seq:
...
#     settings = env_settings()
#     eps = 1e-16
#     category_list = ['NO', 'PO', 'TO', 'HO', 'MB', 'LI', 'HI', 'AIV', 'LR', 'DEF', 'BC', 'SA', 'CM', 'TC', 'FL', 'OV', 'FM', 'SV', 'ARV' ]
#     category_label_path = '/disk3/data/gd/lasher/AttriSeqsTxt'
#     result_plot_path = os.path.join(settings.result_plot_path, report_name)
...
#         category_seq_indices = []

#         # 筛选符合当前类别的序列
#         for seq_id, seq in enumerate(dataset):
#             category_file_path = os.path.join(category_label_path, seq.name + '.txt')
#             if not os.path.isfile(category_file_path):
#                 continue

#             with open(category_file_path, 'r') as f:
#                 category_flags = list(map(int, f.readline().strip().split(',')))

#             category_index = category_list.index(category)
#             if category_flags[category_index] == 1:
#                 category_seq_indices.append(seq_id)

#         # 初始化用于当前类别的张量
#         num_seqs = len(category_seq_indices)
#         avg_overlap_all = torch.zeros((num_seqs, len(trackers)), dtype=torch.float64)
#         ave_success_rate_plot_overlap = torch.zeros((num_seqs, len(trackers), threshold_set_overlap.numel()), dtype=torch.float32)
#         ave_success_rate_plot_center = torch.zeros((num_seqs, len(trackers), threshold_set_center.numel()), dtype=torch.float32)
#         ave_success_rate_plot_center_norm = torch.zeros((num_seqs, len(trackers), threshold_set_center.numel()), dtype=torch.float32)
#         valid_sequence = torch.ones(num_seqs, dtype=torch.uint8)
        
#         # 对当前类别的序列计算评价指标
#         for idx, seq_id in enumerate(category_seq_indices):
#             seq = dataset[seq_id]
#             anno_bb = torch.tensor(seq.ground_truth_rect)
#             target_visible = torch.tensor(seq.target_visible, dtype=torch.uint8) if seq.target_visible is not None else None
...
#                 avg_overlap_all[idx, trk_id] = err_overlap[valid_frame].mean()
...
#                 ave_success_rate_plot_overlap[idx, trk_id, :] = (err_overlap.view(-1, 1) > threshold_set_overlap.view(1, -1)).sum(0).float() / seq_length
#                 ave_success_rate_plot_center[idx, trk_id, :] = (err_center.view(-1, 1) <= threshold_set_center.view(1, -1)).sum(0).float() / seq_length
#                 ave_success_rate_plot_center_norm[idx, trk_id, :] = (err_center_normalized.view(-1, 1) <= threshold_set_center_norm.view(1, -1)).sum(0).float() / seq_length

#         print(f'\n\nComputed results for category {category} over {num_seqs} sequences')

#         seq_names = [dataset[idx].name for idx in category_seq_indices]
...
#         with open(os.path.join(result_plot_path, f'{category}_eval_data.pkl'), 'wb') as fh:
#             pickle.dump(eval_data, fh)
