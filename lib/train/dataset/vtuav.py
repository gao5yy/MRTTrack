...
class VTUAV(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        self.init_idx = np.load(os.path.abspath(os.path.join('/disk0/gd/home/TBSI-main/', 'init_frame.npy')),
    ...
    def _read_bb_anno(self, seq_path):
        bb_anno_file_v = os.path.join(seq_path, "rgb.txt")
        gt_v = np.loadtxt(bb_anno_file_v).astype(np.float32)
    
        bb_anno_file_i = os.path.join(seq_path, "ir.txt")
        gt_i = np.loadtxt(bb_anno_file_i).astype(np.float32)
        return torch.tensor(gt_v), torch.tensor(gt_i)
    ...
    def get_sequence_info(self, seq_id):          # 1
        seq_path = self._get_sequence_path(seq_id)  #/disk3/data/gd/vtuav/pedestrian_197
        
        bbox_v, box_i = self._read_bb_anno(seq_path)
        valid = (bbox_v[:, 2] > 0) & (bbox_v[:, 3] > 0)
        visible = torch.ByteTensor([1 for v in range(len(bbox_v))])
        visible_ratio = torch.ByteTensor([1 for v in range(len(bbox_v))])       

        return {'bbox': bbox_v,  'box_i' : box_i, 'valid': valid, 'visible': visible, 'visible_ratio': visible_ratio}
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        if seq_name in self.init_idx:
            ...
            init_idx = 0
        nz = 6
        vis_frame_names = os.path.join(seq_path, 'rgb', str(frame_id * 10 + init_idx).zfill(nz) + '.jpg')
        inf_frame_names = os.path.join(seq_path, 'ir', str(frame_id * 10 + init_idx).zfill(nz) + '.jpg')
        return vis_frame_names, inf_frame_names
