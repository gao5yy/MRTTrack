...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, LasHeR, RGBT, VTUAV
from lib.train.dataset import Lasot_lmdb, Got10k_lmdb, MSCOCOSeq_lmdb, ImagenetVID_lmdb, TrackingNet_lmdb
from lib.train.data import sampler, opencv_loader, processing, LTRLoader, sampler_uav
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "LasHeR_train",  "LasHeR_test", "RGBT234", "VTUAV"]
        if name == "LASOT":
        ...
        if name == "RGBT234":
            datasets.append(RGBT(settings.env.RGBT234_dir, split='train', image_loader=image_loader))
        if name == "VTUAV":
            datasets.append(VTUAV(settings.env.UAV_RGBT_dir, split='train', image_loader=image_loader))
...
def build_dataloaders(cfg, settings):
    ...
    if cfg.DATA.TRAIN.DATASETS_NAME[0] == "VTUAV":
        dataset_train = sampler_uav.TrackingSampler(datasets=names2datasets(cfg.DATA.TRAIN.DATASETS_NAME, settings, opencv_loader),
    ...
    if cfg.DATA.TRAIN.DATASETS_NAME[0] == "VTUAV":
        dataset_val = sampler_uav.TrackingSampler(datasets=names2datasets(cfg.DATA.VAL.DATASETS_NAME, settings, opencv_loader),
